...
const ROLE_C: Record<string, string> = { ADMIN: '#ef4444', HQ_MANAGER: '#8b5cf6', BRANCH_MANAGER: '#3b82f6' };
...
export default function HQDashboard() {
  ...
  const switchTab = (t: Tab) => {
    setTab(t);
    if (t === 'inventory') api.get('/hq/inventory').then(r => setInventory(r.data));
    if (t === 'staff') api.get('/hq/staff/elevated').then(r => setElevatedStaff(r.data));
    if (t === 'branch-mgmt') api.get('/admin/branches').then(r => setAllBranches(r.data));
  ...
  const loadSales = async () => {
    ...
    if (salesFilter.branchId) p.set('branchId', salesFilter.branchId);
    if (salesFilter.from) p.set('from', salesFilter.from);
    if (salesFilter.to) p.set('to', salesFilter.to);
    const r = await api.get(`/hq/reports/sales-data?${p.toString()}`);
    setSalesData(r.data);
  };
  ...
  const createBranch = async () => {
    ...
    api.get('/admin/branches').then(r => setAllBranches(r.data));
    loadOverview();
  };
  ...
  const createStaff = async () => {
    ...
    api.get('/hq/staff/elevated').then(r => setElevatedStaff(r.data));
  };
  ...
  const maxDetailRev = selBranch ? Math.max(...selBranch.revenueByDay.map(d => d.revenue), 1) : 1;
  ...
  return (
    <div style={{ background: '#1a120b', minHeight: '100vh', color: '#f5ede3', fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{
        ...
        padding: '0 36px', display: 'flex', gap: 0, overflowX: 'auto',
      }}>
        {TABS.map(([t, lbl, icon]) => (
      ...
      <main style={{ padding: '36px', maxWidth: 1400, margin: '0 auto' }}>
                ...
                return branches.map(b => (
                  <div key={b.id} style={{ marginBottom: 16 }}>
                    ...
                    <div style={{ height: 10, background: '#3d2e22', borderRadius: 5 }}>
                      <div style={{ height: '100%', width: `${((b.stats?.revenue || 0) / maxR) * 100}%`, background: 'linear-gradient(to right,#ffb77d,#ffd4a8)', borderRadius: 5, transition: 'width 0.6s' }} />
                    </div>
                    <span style={{ fontSize: 11, color: 'rgba(245,237,227,0.35)' }}>{b.stats?.totalOrders || 0} orders · {b.stats?.staffCount || 0} staff</span>
                  </div>
                ));
              })()}
            </div>
        ...
        {tab === 'analytics' && (
          <>
            <p style={{ color: 'rgba(245,237,227,0.5)', fontSize: 14, marginBottom: 24 }}>
              Select a branch to view its detailed graphical performance analytics.
            </p>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(260px, 1fr))', gap: 18 }}>
              {branches.map(b => (
                <div key={b.id} onClick={() => openBranchDetail(b.id)}
                  style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 10, padding: '22px', cursor: 'pointer', transition: 'all 0.25s', position: 'relative' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = '#ffb77d'; (e.currentTarget as HTMLElement).style.transform = 'translateY(-4px)'; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = '#3d2e22'; (e.currentTarget as HTMLElement).style.transform = 'translateY(0)'; }}>
                  <div style={{ position: 'absolute', top: 14, right: 14, width: 8, height: 8, borderRadius: '50%', background: b.isActive ? '#4caf50' : '#ef4444' }} />
                  ...
                  <div style={{ display: 'flex', gap: 24 }}>
                    <div>
                      <p style={{ margin: 0, fontFamily: 'Playfair Display, serif', fontSize: 20, fontWeight: 700, color: '#ffb77d' }}>£{Math.round(b.stats?.revenue || 0).toLocaleString()}</p>
          ...
          <>
              ...
              <div style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 8, padding: '20px 24px' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8, height: 160 }}>
                  {selBranch.revenueByDay.map(d => {
                    const pct = (d.revenue / maxDetailRev) * 100;
                    return (
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 18 }}>
              <div style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 8, padding: '20px 24px' }}>
                ...
                {selBranch.topSelling.slice(0, 6).map((item, i) => (
              ...
              <div style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 8, padding: '20px 24px' }}>
                ...
                {selBranch.staff.map(s => (
                  <div key={s.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                    ...
                    <span style={{ background: (ROLE_C[s.role] ?? '#f59e0b') + '22', color: ROLE_C[s.role] ?? '#f59e0b', border: `1px solid ${(ROLE_C[s.role] ?? '#f59e0b')}44`, borderRadius: 20, padding: '2px 9px', fontSize: 10, fontWeight: 600 }}>
                      {s.role.replace(/_/g, ' ')}
                    </span>
                  </div>
                ))}
        ...
        {tab === 'inventory' && (
          <>
            {LOW_STOCK.length > 0 && (
              <div style={{ background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.3)', borderRadius: 8, padding: '12px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
                  ...
                  {filteredInv.map((item, i) => {
                    ...
                    return (
                      <tr key={item.id} style={{ borderTop: '1px solid #3d2e22', background: i % 2 === 0 ? 'transparent' : 'rgba(255,183,125,0.02)' }}>
                        ...
                        <td style={{ padding: '12px 18px', fontSize: 13, color: 'rgba(245,237,227,0.6)' }}>{item.branch.name} <span style={{ color: 'rgba(245,237,227,0.35)' }}>({item.branch.city})</span></td>
            ...
            <div style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 8, padding: '20px 24px', marginBottom: 20 }}>
              ...
              <div style={{ display: 'flex', gap: 16, alignItems: 'flex-end', flexWrap: 'wrap' }}>
                ...
                {[['from', 'From'], ['to', 'To']].map(([key, lbl]) => (
                  <div key={key}>
                    <label style={{ display: 'block', fontSize: 11, color: 'rgba(245,237,227,0.5)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.08em' }}>{lbl}</label>
                    <input type="date" value={(salesFilter as any)[key]} onChange={e => setSalesFilter(p => ({ ...p, [key]: e.target.value }))}
                ...
                <button onClick={loadSales} style={{ background: '#ffb77d', color: '#1a120b', border: 'none', borderRadius: 6, padding: '9px 20px', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
              ...
              <>
                ...
                <div style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 8, padding: '20px 24px' }}>
                  ...
                  <textarea value={reportDraft} onChange={e => setReportDraft(e.target.value)}
                    placeholder="Add executive summary, observations, or action items here…" rows={5}
                  ...
                  <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
                    <button onClick={() => alert('Draft saved!')} style={{ background: '#ffb77d', color: '#1a120b', border: 'none', borderRadius: 6, padding: '9px 20px', fontWeight: 700, fontSize: 13, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        {tab === 'branch-mgmt' && (
          <>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
              <button onClick={() => setShowCreateBranch(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#ffb77d', color: '#1a120b', border: 'none', borderRadius: 6, padding: '10px 20px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px,1fr))', gap: 18 }}>
              {allBranches.map(b => (
                <div key={b.id} style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 10, padding: '22px', position: 'relative' }}>
                  ...
                  <h4 style={{ fontFamily: 'Playfair Display, serif', fontSize: 18, margin: '0 0 4px', color: '#fff' }}>{b.name}</h4>
                  <p style={{ fontSize: 12, color: 'rgba(245,237,227,0.45)', margin: '0 0 2px' }}>{b.city} · {b.address}</p>
                  {b.phone && <p style={{ fontSize: 12, color: 'rgba(245,237,227,0.35)', margin: '0 0 16px' }}>{b.phone}</p>}
                  <div style={{ display: 'flex', gap: 20, marginBottom: 16 }}>
                    <div>
                      <p style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#fff', fontFamily: 'Playfair Display, serif' }}>{b._count?.users ?? 0}</p>
                      <p style={{ margin: 0, fontSize: 10, color: 'rgba(245,237,227,0.35)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Staff</p>
                    </div>
                    <div>
                      <p style={{ margin: 0, fontSize: 18, fontWeight: 700, color: '#fff', fontFamily: 'Playfair Display, serif' }}>{b._count?.orders ?? 0}</p>
          ...
          <>
            <div style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.25)', borderRadius: 8, padding: '12px 18px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <span style={{ fontSize: 13, color: 'rgba(245,237,227,0.7)' }}>
                HQ Managers can add or remove <strong style={{ color: '#fff' }}>System Admins</strong> and <strong style={{ color: '#fff' }}>Branch Managers</strong> only.
              </span>
            </div>
            <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 20 }}>
              <button onClick={() => setShowCreateStaff(true)} style={{ display: 'flex', alignItems: 'center', gap: 8, background: '#ffb77d', color: '#1a120b', border: 'none', borderRadius: 6, padding: '10px 20px', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20 }}>
              {[
                { role: 'ADMIN', label: 'System Admins', color: '#ef4444', icon: 'admin_panel_settings' },
                { role: 'BRANCH_MANAGER', label: 'Branch Managers', color: '#3b82f6', icon: 'manage_accounts' },
              ].map(({ role, label, color, icon }) => (
                <div key={role} style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 8, overflow: 'hidden' }}>
                  <div style={{ padding: '14px 20px', borderBottom: '1px solid #3d2e22', display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <h4 style={{ margin: 0, fontSize: 14, color: '#fff', fontFamily: 'Playfair Display, serif' }}>{label}</h4>
                    <span style={{ marginLeft: 'auto', background: color + '22', color, borderRadius: 20, padding: '2px 10px', fontSize: 11, fontWeight: 600 }}>
                      {elevatedStaff.filter(s => s.role === role).length}
                    </span>
                  </div>
                  {elevatedStaff.filter(s => s.role === role).map(s => (
                    <div key={s.id} style={{ padding: '13px 20px', borderBottom: '1px solid rgba(61,46,34,0.5)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      <div>
                        <p style={{ margin: 0, fontSize: 14, color: '#fff', fontWeight: 500 }}>{s.name}</p>
                        <p style={{ margin: 0, fontSize: 12, color: 'rgba(245,237,227,0.4)' }}>{s.email}</p>
                        {s.branch && <p style={{ margin: 0, fontSize: 11, color }}>{s.branch.name}</p>}
                      </div>
                  ...
                  {elevatedStaff.filter(s => s.role === role).length === 0 && (
                    <div style={{ padding: '24px 20px', textAlign: 'center', color: 'rgba(245,237,227,0.3)', fontSize: 13 }}>No {label.toLowerCase()} found</div>
                  )}
                </div>
              ))}
            </div>
      ...
      {showCreateBranch && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 10, padding: 32, width: 460 }}>
            ...
            {[['name','Branch Name *','Steakz Birmingham'],['city','City *','Birmingham'],['address','Address *','15 Broad Street, B1 2AA'],['phone','Phone','+44 121 000 0000'],['email','Email','birmingham@steakz.co.uk']].map(([key, lbl, ph]) => (
              <div key={key} style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', fontSize: 11, color: 'rgba(245,237,227,0.6)', marginBottom: 6, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{lbl}</label>
                <input value={(branchForm as any)[key]} onChange={e => setBranchForm(p => ({ ...p, [key]: e.target.value }))} placeholder={ph}
      ...
      {showCreateStaff && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200 }}>
          <div style={{ background: '#271e16', border: '1px solid #3d2e22', borderRadius: 10, padding: 32, width: 460 }}>
            ...
            {[['name','text','Full Name *','Jane Smith'],['email','email','Email *','jane@steakz.co.uk'],['password','password','Password *','••••••••']].map(([key, type, lbl, ph]) => (
              <div key={key} style={{ marginBottom: 14 }}>
                <label style={{ display: 'block', fontSize: 11, color: 'rgba(245,237,227,0.6)', marginBottom: 6, letterSpacing: '0.06em', textTransform: 'uppercase' }}>{lbl}</label>
                <input type={type} value={(staffForm as any)[key]} onChange={e => setStaffForm(p => ({ ...p, [key]: e.target.value }))} placeholder={ph}
                  style={{ width: '100%', background: '#1a120b', border: '1px solid #3d2e22', borderRadius: 6, color: '#f5ede3', padding: '10px 14px', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
              </div>
            ))}
